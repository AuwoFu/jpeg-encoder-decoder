...
def interpolateTable(q, targetSize):

    min = q.min()
    max = q.max()
    X = np.linspace(min, max, 8)
    Y = np.linspace(min, max, 8)
    x, y = np.meshgrid(X, Y)
    f = interpolate.interp2d(x, y, q, kind='linear')

    # use linspace so your new range also goes from 0 to 3, with 8 intervals
    Xnew = np.linspace(min, max, targetSize)
    Ynew = np.linspace(min, max, targetSize)
...
def load_quantization_table(component, type=0, block_size=8):
    ...
    if type == 1:
        ...
        if component == 'lum':
            ...
            raise ValueError((
                                 ...
                                 "but '{comp}' was found").format(comp=component))
    if block_size!=8:
        q=interpolateTable(q,block_size)
    return q
